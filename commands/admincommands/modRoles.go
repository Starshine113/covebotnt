...
// ModRoles ...
func ModRoles(ctx *crouter.Ctx) (err error) {
	...
	if len(ctx.Args) >= 1 {
		...
		case "add":
			...
			err = addModRole(ctx, ctx.Message.GuildID, role.ID)
			if err != nil {
				ctx.CommandError(err)
				return nil
			}
		...
		case "remove", "delete":
			...
			err = delModRole(ctx, ctx.Message.GuildID, role.ID)
			if err != nil {
				ctx.CommandError(err)
				return nil
			}
	...
	roleEmbed, err := getModRoles(ctx, ctx.Message.GuildID)
	if err != nil {
		ctx.CommandError(err)
		return nil
	}
...
func addModRole(ctx *crouter.Ctx, guildID, roleID string) (err error) {
	...
	commandTag, err := ctx.Database.Pool.Exec(context.Background(), "update public.guild_settings set mod_roles = $1 where guild_id = $2", roles, guildID)
	if err != nil {
		return err
	}
...
func delModRole(ctx *crouter.Ctx, guildID, roleID string) (err error) {
	gs, err := ctx.Database.GetGuildSettings(guildID)
	if err != nil {
		return err
	}
...
func getModRoles(ctx *crouter.Ctx, guildID string) (*discordgo.MessageEmbed, error) {
	var roles []string
